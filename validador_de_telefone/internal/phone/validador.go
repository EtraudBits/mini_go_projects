...
import "unicode" // Permite indentificar digitos numeros, ignorando parentes, espaços e hifen.
...
func ValidadorPhone(phone string) error { // ValidadorPhone passa uma string e retorna um erro - ValidadorPhone takes a string and returns an error
	...
	if phone == "" {
		return ErrEmptyPhone // 1 - Não pode ser vazio retorna ErrEmptyPhone da pasta errors.go / 1 - Cannot be empty returns ErrEmptyPhone from errors.go folder
		// validação mais simples primeiro, falhou? retorna o erro.
	}
	...
	if len(digits) != 11 { // se o temanho (len) do slice for diferente de 11 - if the length (len) of the slice is different from 11.
		return ErrInvalidDigits // retorna ErrInvalidDigits da pasta errors.go - returns ErrInvalidDigits from errors.go folder.
	}
	...
	if !validateDDDs[ddd] { // se o DDD não estiver no mapa - if the DDD is not in the map
		return ErrInvalidDDD // retorna ErrInvalidDDD da pasta errors.go - returns ErrInvalidDDD from errors.go folder
	}
	...
	if digits[2] != '9' { // se o terceiro digito (índice 2) não for 9 - if the third digit (index 2) is not 9
		return ErrInvalidStart // retorna ErrInvalidStart da pasta errors.go - returns ErrInvalidStart from errors.go folder
	}
